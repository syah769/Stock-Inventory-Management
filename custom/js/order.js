...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			if(category == "") {
				$("#category").after('<p class="text-danger"> The Category field is required </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && category && paid && discount && paymentType && paymentStatus) {
...
function subAmount() {
	...
	// vat
	var vat = (Number($("#subTotal").val())/100) * 6;
	vat = vat.toFixed(2);
	$("#vat").val(vat);
	$("#vatValue").val(vat);
